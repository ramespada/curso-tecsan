...
\pause      
 donde R es la radiación incidente:\footnote{$S_o\approx1366 \pm 7 W/m^2$: cte. Solar. $\tau$: transmisividad de la atmos, depende del recorrido de los rayos (i.e la latitud) y de la nubosidad.}
...
La teoría de similitud es un método para desarrollar ecuaciones empíricas, nos va a servir para estimar perfiles verticales de varios parámetros atmosféricos. %Similarity theory is a method for developing empirical equations to estimate the vertical profile of various atmospheric parameters.

El objetivo es definir relaciones que apliquen a distintas condiciones atmosféricas. %The goal is to define universal relationships that apply under a variety of  atmospheric conditions and locations (once the uniqueness of the location’s surface and thermodynamical properties are understood)

Las relaciones de similitud generalmente se diseñan para usar en sistemas en estado estacionario/equilibrio. %Similarity relationships are usually designed to apply to steady-state , time-independent situations (equilibrium)

%AERMOD usa relaciones de similitud para carectarizar la CLP %AERMOD uses similarity relationships to characterize the atmospheric boundary layer
    
Los principales parámetros necesarios para definir su unicidad para la construcción de perfiles son: %   The parameters of a location needed to define its uniqueness for the purposes of constructing vertical profiles include mainly:
\begin{itemize}
...
\begin{frame}{Ejemplo: Perfil de vientos en capa de superficie}
...
\begin{frame}{Velocidad de Fricción}{Medida de la fuerza de corte}
